...
club_comments_bp = Blueprint('club_comments', __name__, url_prefix='/api/clubs/<int:club_id>/comments')
...
def post_comment(club_id):
    ...
    c = ClubComment(club_id=club_id, user_id=user_id, content=data['content'])
    db.session.add(c)
    db.session.commit()
